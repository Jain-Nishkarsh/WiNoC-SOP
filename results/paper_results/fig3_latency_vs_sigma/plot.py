#!/usr/bin/env python3
"""
Figure 3: Average Latency vs. Spatial Hotspotness (σ)
...
Fixed parameters: H=0.5, PIR=0.025
Sweeps σ: [0.1, 0.3, 0.5, 0.7, 1.0, 1.5, 2.0, 3.0]

Uses temp YAML configs for sigma parameter.
...
SIGMA_LIST = [0.1, 0.3, 0.5, 0.7, 1.0, 1.5, 2.0, 3.0]
FIXED_HURST = 0.5
FIXED_PIR = 0.025
NUM_SEEDS = 10

# Output files
CSV_OUTPUT = SCRIPT_DIR / "sigma_sweep_data.csv"
...
def create_temp_config(base_config: dict, sigma: float, protocol_name: str) -> str:
    """Create temp YAML config with modified sigma."""
    config = base_config.copy()
    config['traffic_soteriou_sigma'] = sigma
    
    # Write to temp file
    fd, temp_path = tempfile.mkstemp(suffix='.yaml', prefix=f'noxim_{protocol_name}_sigma{sigma}_')
...
def run_sweep(base_config_path: str, protocol_name: str):
    """Run sigma sweep for a single protocol."""
    ...
    results = {"sigma": [], "latency": []}
    
    for sigma in SIGMA_LIST:
        # Create temp config with this sigma
        temp_config_path = create_temp_config(base_config, sigma, protocol_name.replace(" ", "_"))
        ...
        print(f"  σ={sigma:.1f}: Latency={gmean_latency:.2f}")
...
def save_to_csv(all_results: dict):
    ...
    for protocol, data in all_results.items():
        for sigma, latency in zip(data["sigma"], data["latency"]):
            rows.append({
                "protocol": protocol,
                "sigma": sigma,
                "latency": latency
...
def load_from_csv() -> dict:
    ...
    for protocol in df["protocol"].unique():
        ...
        all_results[protocol] = {
            "sigma": protocol_data["sigma"].tolist(),
...
def plot_results(all_results: dict):
    """Generate IEEE-quality latency vs sigma plot."""
    ...
    ax.set_xlabel(r"Spatial Hotspotness $\sigma$")
    ...
    output_png = SCRIPT_DIR / "fig3_sigma.png"
    output_eps = SCRIPT_DIR / "fig3_sigma.eps"
...
def main():
    print("=" * 60)
    print("Figure 3: Latency vs. Spatial Hotspotness (σ)")
    print("=" * 60)
    print(f"σ range: {SIGMA_LIST}")
    print(f"Fixed: H={FIXED_HURST}, PIR={FIXED_PIR}")
